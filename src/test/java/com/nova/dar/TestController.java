...
public class TestController {
	...
	public void setUp() throws Exception {
		...
		this.task = new MyTask("Task 1", 1);
		this.task2 = new MyTask("Task 2", 2);
		this.task3 = new MyTask( "Task 3", 1);
		
		String reponse =  this.mockMvc.perform(post("/tasks").accept(MediaType.APPLICATION_JSON)
	...
	public void createTaskTest() throws Exception {
		...
		String response = this.mockMvc.perform(post("/tasks").accept(MediaType.APPLICATION_JSON)
				...
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$._description").value("Task 4"))
				.andExpect(jsonPath("$._state").value(0))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
	...
	public void getTaskById() throws Exception {
		String response = this.mockMvc.perform(get("/getTaskById").param("id", task.get_id().toString()).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$._id").value(task.get_id()))
				.andReturn().getResponse().getContentAsString();
	...
	public void updateTask() throws Exception {
		...
		//Update task3
		this.mockMvc.perform(put("/tasks").accept(MediaType.APPLICATION_JSON)
				.content(objectmapper.writeValueAsString(taskmodif))
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
		//Check task3
		String response1 = this.mockMvc.perform(get("/getTaskById").param("id", taskmodif.get_id().toString()).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$._id").value(taskmodif.get_id()))
				.andReturn().getResponse().getContentAsString();
	...
	public void deleteTaskById() throws Exception {
		String response = this.mockMvc.perform(delete("/deleteTaskById").param("id", task.get_id().toString()).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString();
